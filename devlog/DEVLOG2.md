...
## Run 27 (128 units, 256 warmup, all LFO rates)

Same architecture as run 23 (128 GRU units, concat conditioning, 256 warmup, 100Hz floor). The only change: `VARIABLE_TRAIN_FILES` extended from one LFO rate (5Hz) to all five available rates (1, 2, 5, 10, 20 Hz), all sweeping 100Hz to 10kHz. `VARIABLE_VAL_FILES` updated the same way. Static training data unchanged. Ran all 300 epochs, best val_loss 0.0000.

### Static ESR (dB) -- bench and ruin

| Freq | Run 23 bench | Run 27 bench | Run 23 ruin | Run 27 ruin |
|------|-------------:|-------------:|------------:|------------:|
| 100 Hz | -34.6 | -42.6 | -29.7 | -36.5 |
| 250 Hz | -39.7 | -45.4 | -36.9 | -42.4 |
| 500 Hz | -42.5 | -48.3 | -41.9 | -44.9 |
| 1 kHz | -45.0 | -50.0 | -45.4 | -48.9 |
| 16 kHz | -51.1 | -53.1 | -51.6 | -53.9 |
| 20 kHz | -52.2 | -54.9 | -52.9 | -55.3 |

### Dynamic ESR (dB) -- ruin

| Rate | Run 23 | Run 27 | Delta |
|------|-------:|-------:|------:|
| 1 Hz | -40.8 | -46.3 | +5.5 |
| 2 Hz | -40.4 | -46.0 | +5.6 |
| 5 Hz | -38.9 | -45.8 | +6.9 |
| 10 Hz | -36.1 | -45.0 | +8.9 |
| 20 Hz | -31.8 | -42.7 | +10.9 |

### Analysis

The gains are larger than expected. Static bench improved 5-8 dB across the board, including +8 dB at 100 Hz -- a frequency the warmup period-visibility analysis predicted would be hard to improve. The likely mechanism: LFO sweeps pass through low frequencies repeatedly in varying contexts, giving the model far more low-frequency exposure than the static training grid did at those frequencies.

Rate sensitivity is essentially gone. Run 23 spanned 9 dB from 1 to 20 Hz on ruin. Run 27 spans 3.6 dB over the same range (-46.3 to -42.7 dB), matching the rate-invariance that run 24 (LFO-only) achieved but without its 10 kHz frequency ceiling.

Run 27 is the new deployed model. `test_passthrough.csd` and `test_midi_saw.csd` updated to point at `models/27_moog_variable_allrates_128u_w256/weights.json`. Polyphony ceiling unchanged (6 voices), since architecture and hidden size are identical to run 19 and run 23.

---

## Stale DLL issue

After updating the `.csd` files to run 27, the passthrough test produced full dry signal with no filtering. Run 19 showed the same symptom, ruling out the model weights. Root cause: the `moognn.dll` binary on disk was compiled from an older version of `moognn.cpp` and did not match the current source. A git branch merge does not rebuild compiled artifacts. The fix was a manual rebuild:

```
cmake --build build --config Release --target moognn
```

After rebuilding, both run 19 and run 27 filtered correctly. Rebuild moognn.dll any time `moognn.cpp` or its dependencies change, or after any branch merge that touches C++ source.
...
**Moog filter opcode (`moognn`)**
- Training data generated by `src/moogGen/` (RK4 Moog ladder, 8× oversampling) from `audio/bench_mono.wav` and `audio/testSound_mono.wav`, filtered at a grid of static cutoffs plus multi-rate LFO sweeps (`audio/filteredOutput/`)
- Deployed model: `models/27_moog_variable_allrates_128u_w256/weights.json` (128u GRU, concat conditioning, static + 5-rate LFO training)
- Frontier models for benchmarking: `models/25_moog_100-20k_32u/`, `models/16_moog_100-20k_64u_w256/`, `models/19_moog_100-20k_128u_w256/`, `models/26_moog_100-20k_256u/`
- Opcode source: `src/csound_opcode/moognn.cpp` — templated across hidden sizes, registers `moognn32/64/128/256`
- Test scores: `csound/test_passthrough.csd`, `csound/test_midi_saw.csd`
- Eval scripts: `python/eval_param_model.py` (static), `python/eval_dynamic.py` (LFO/sweep)

**Distortion opcode (`distnn`)**
- Training data: `audio/updatedDistortion/` — breadboard loopback recordings at 0 dB and -10 dB, dry/wet pairs aligned via `python/align_audio.py`
- Deployed model: `models/dist_12_gru128_l1x10/weights.json` (128u GRU, MR-STFT + L1×10 loss)
- Opcode source: `src/csound_opcode/distnn.cpp`
- Test scores: `csound/test_distnn.csd`, `csound/test_midi_distnn.csd`
- Eval script: `python/eval_distortion.py` (multi-pair ESR table + spectrograms), `python/eval_asr.py` (aliasing)

**Benchmark**
- DSP baselines: `bench/bench_dsp.cpp` (standalone RKSimulationMoog), `src/csound_opcode/rkmoog.cpp` (same algorithm as a Csound opcode)
- Harness: `bench/bench_opcode.csd`, `bench/run_benchmarks.py`, `bench/plot_results.py`
- Results: `bench/results/opcode.csv`, `bench/results/dsp_5600x_kr.csv`
- Figures: `bench/results/cost_frontier.png`, `bench/results/polyphony_plot.png`, `bench/results/headline_table.md`

### Narrative

1. **Motivation.** Csound has no mechanism for data-driven effect modeling. Native opcodes like `moogladder` implement fixed algorithms; hardware effects have no DSP equivalent at all. The goal is a general pipeline: record dry/wet pairs, train a GRU model, deploy as a real-time opcode.

2. **Opcode framework.** CPOF (`src/csound_opcode/`) with RTNeural (xsimd + AVX2) for inference. The same Conv1d + GRU + Dense + skip architecture serves both effects. Key implementation details: JSON weight loading at i-time, `moognn_preload` to avoid blocking the first note, fade-in to cover GRU cold-start convergence, templated hidden sizes to enable the cost frontier.

3. **Moog filter: training data and architecture ablation.** RK4 training targets, parameterized by log-normalized cutoff knob. Ablation across warmup length, unit count, k2h0, LayerNorm, LSTM vs GRU, and FiLM conditioning. Key findings: 256-sample warmup is the practical floor; FiLM does not improve dynamic tracking because the conditioning problem is in the recurrent state, not the input pathway.

4. **Moog filter: variable training data as the key insight.** Static-only training (run 19) degrades under fast modulation — the GRU must integrate the knob over time to infer the filter mode, and fast sweeps don't give it enough settling time. Adding multi-rate LFO sweeps to training (run 27) eliminates rate sensitivity: 1–20 Hz dynamic range shrinks from 9 dB to 3.6 dB, static accuracy improves 5–8 dB across the board, and the 100 Hz warmup constraint partially relaxes.

5. **Distortion: hardware recording target.** No DSP reference exists for the breadboard pedal. Key challenges: training data diversity (loopback recording methodology), loss function (MR-STFT + L1 eliminates the waveform-level ESR shortcut for larger models), and recording alignment (`align_audio.py`). Aliasing ruled out as the held-out gap cause via ASR measurement. Final model: dist_12 at -29.2 dB held-out ESR.

6. **Benchmark.** moognn_128u runs at 16.9% CPU per voice (6-voice polyphony), 24× more expensive than `moogladder` and 6× more expensive than the RK4 reference. The cost is the price of generality: the same pipeline that models the Moog from simulation data models the hardware pedal from recordings, with no algorithm changes.